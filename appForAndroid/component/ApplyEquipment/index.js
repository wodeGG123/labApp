...
export default class ApplyEquipment extends Component {
  ...
  _submit(){
    ...
    .then((data)=>{
      if(data.code == '0000'){
        Alert.alert('申请成功！',null,[{text: 'OK',onPress:()=>{
            this.props.navigator.pop()
        }}]);

      }else{
        Alert.alert(data.data.berror[0],null,[{text: 'OK',onPress:()=>{
      ...
      <View style={styles.container}>
        ...
        <Modal
          ...
          <View style={{backgroundColor:'white'}}>
              ...
              <TouchableHighlight style={styles.submit} activeOpacity={0.6} underlayColor={'#7b7dea'} onPress={async ()=> {
                ...
                try {
                  const {action, year, month, day} = await DatePickerAndroid.open({
                    ...
                    maxDate:new Date((+this.state.date)+1000*24*60*60*parseInt(30)),
                  });

                  if (action === DatePickerAndroid.dismissedAction) {
                    // Selected year, month (0-11), day
                  }else{
                    this.onDateStartChange(new Date(year,month,day));
                  }
                } catch ({code, message}) {
                ...
                <Text style={styles.submitText}>
                  {dateFormat(this.state.dateStart,'yyyy年mm月dd日')}
                </Text>
              </TouchableHighlight>
          </View>
          <View style={{backgroundColor:'white'}}>
              ...
              <TouchableHighlight style={styles.submit} activeOpacity={0.6} underlayColor={'#7b7dea'} onPress={async ()=> {
                ...
                try {
                  ...
                  if (action === DatePickerAndroid.dismissedAction) {
                    ...
                    this.onDateEndChange(new Date(year,month,day));
                  }
                } catch ({code, message}) {
                  console.warn('Cannot open date picker', message);
                }
                ...
                <Text style={styles.submitText}>
                  {dateFormat(this.state.dateEnd,'yyyy年mm月dd日')}
                </Text>
              </TouchableHighlight>
